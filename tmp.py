...
def main(model_name="mcdrop", arch_name="lenet5", max_epochs=20, data_set="mnist", resume=False, exp_num=0):
    batch_size = 128
    train_loader, test_loader, ood_loader, test_ood_loader, post_test_ood_loader, n_channel, n_classes = prepare_data(data_set, batch_size,
                                                                                                iscuda)
    ...
    # Pick architecture
    if arch_name == "lenet5":
        arch = LeNet5(n_channel, n_classes, isvb=(model_name == "vb"), has_context=(model_name == "etp"))
    elif arch_name == "resnet18":
        arch = Resnet18(n_channel, n_classes, isvb=(model_name == "vb"), has_context=(model_name == "etp"))

    # Pick model
    if model_name == "mcdrop" or model_name == "vb":
        model = BayesianNeuralNet(arch, isvb=(model_name == "vb"))

    elif model_name == "evae":
        model = EVAE(arch)
    ...
    if arch_name == "lenet5":
        ...
        if model_name == "rpn":
            lrate = 5e-3
            opt = th.optim.SGD(model.parameters(), lr=lrate, momentum=0.9, weight_decay=5e-3)
        else:
            lrate = 0.05
            opt = th.optim.SGD(model.parameters(), lr=lrate, momentum=0.9, weight_decay=5e-4)
    ...
    if resume:
        if os.path.exists(f'runs/baselines/{arch_name}/{data_set}/{model_name}/replication_{exp_num + 1}/checkpoint.pt'):
            model.load_state_dict(th.load(
                f'runs/baselines/{arch_name}/{data_set}/{model_name}/replication_{exp_num + 1}/checkpoint.pt')[
                                      "model_state_dict"])
            opt.load_state_dict(th.load(
                f'runs/baselines/{arch_name}/{data_set}/{model_name}/replication_{exp_num + 1}/checkpoint.pt')[
    ...
    for epoch in range(1, max_epochs + 1):
        start = time.time()
        opt = train(model, train_loader, ood_loader, epoch, opt, iscuda=iscuda, verbose=True)
    ...
    result["test"] = get_results(model, test_loader, ood_loader, test_ood_loader, post_test_ood_loader, iscuda,
                                 batch_size)
    result["train"]["epoch_time"] = timedelta(seconds=result["train"]["epoch_time"] / max_epochs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="mcdrop", choices=["mcdrop", "vb", "evae", "etp", "edl", "rpn", "ts"])
    parser.add_argument("--arch", default="lenet5", choices=["lenet5", "resnet18"])
    parser.add_argument("--dataset", default="mnist", choices=["mnist", "fashion", "c10", "c100", "svhn"])
    parser.add_argument("--max_epochs", type=int, default=10)
    parser.add_argument("--max_replication", type=int, default=5)
    parser.add_argument('--resume', default=True, type=lambda x: (str(x).lower() == 'true'))
    ...
    for exp_num in range(max_replication):
        try:
            start = time.time()
            result, model, optimizer = main(arch_name=arch, model_name=model_name, max_epochs=max_epochs,
                                            data_set=dataset, resume=args.resume, exp_num=exp_num)
            ...
            os.makedirs(f'runs/baselines/{arch}/{dataset}/{model_name}/{replication_name}/', exist_ok=True)
            th.save(state,
                    f'runs/baselines/{arch}/{dataset}/{model_name}/{replication_name}/checkpoint.pt')

            with open(f'runs/baselines/{arch}/{dataset}/{model_name}/{replication_name}/train.txt', "w") as f:
                json.dump(result["train"], f, default=str)

            with open(f'runs/baselines/{arch}/{dataset}/{model_name}/{replication_name}/test.txt', "w") as f:
